<template>
  <div v-if="!item.hidden" class="menu-wrapper">
    ...
    >
      <app-link
        v-if="
          (onlyOneChild.meta && (power.includes(item.name) || $DevelopmentRoute.includes(item.name))) ||
          ignorePermissions
        "
    ...
    <el-submenu v-else ref="subMenu" :index="resolvePath(item.path)" popper-append-to-body>
      ...
      <template v-for="child in item.children">
        <sidebar-item
          v-if="$DevelopmentRoute.includes(child.name) || power.includes(child.name) || ignorePermissions"
          :key="child.path"
          :is-nest="true"
          :item="child"
