    ...
    >
      ...
      <el-form-item prop="verificationCode" class="code">
        ...
        <el-input
          ...
          @keyup.enter.native="submit"
        />
        <img v-if="imgUrl" class="codeImg" :src="imgUrl" alt="" @click="getCodeImg" />
      </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      loginRules: {
        ...
        verificationCode: [{ required: true, message: '请输入合格验证码' }]
      },
      loading: false,
  ...
  methods: {
    ...
    async getCodeImg() {
      const key = nanoid()
      const res = await this.axios.get(`/noToken/createImage`, {
        responseType: 'blob',
        params: {
          key
        }
      ...
      this.loginForm.verificationKey = key
