 ...
 * @LastEditTime: 2020-04-16 19:34:08
 -->
<template>
  <div style="height: 100%">
    <el-card class="box-card">
      <div slot="header" class="flex">
        ...
        <div v-if="!versionSatus && !autoChangeModel">
...
import ModelApi from '@/api/model/modelVersion'
  ...
  methods: {
      ...
      */
      ...
      if (componentList.length > 0) {
        for (let item of componentList) {
          let list = []
          // bug#1641 大多数模块不需要查询属性，解决大多数模块的框选性能问题 申豪强 2020-04-16
          if (this.bindType) {
    ...
    showModal() {
      ...
      if (_.isEmpty(this.singleModels)) {
        obj = _.cloneDeep(this.majorModels)
      } else if (_.isEmpty(this.majorModels)) {
        obj = _.cloneDeep(this.singleModels)
      } else {
        //对象交集、数组交集
    ...
    checkedNodesFilter(checkedNodes) {
      ...
      checkedNodes.forEach(e => {
        if (e.modelId) {
          e.modelId = e.modelId.replace('_null', '')
        }
      })
      modelIds.forEach(id => {
        let ids = checkedNodes
          .filter(item => item.components && item.components.length > 0 && item.modelId === id)
          .map(item => item.components)
        let dbids = this.flatten(ids)
        obj[id] = dbids
      })
    ...
    load3dviewer(arr) {
      ...
      this.bimfishApp.init(() => {
        this.bimfishApp.showModel(arr, () => {
          if (this.hideAll) this.bimfishApp.hideAll()
        })
      })
    },
    // 获取模型数据
    async getProjectTreeData() {
      let res
      if (this.flag && this.flag === 'version') {
        let { id } = this.$route.params
        res = await ModelApi.getEntityTree(id)
      } else {
        res = await Api.getProjectTreeData(localStorage.getItem('projectId'))
      }
      if (!res) {
        return
      }
